...
import { bigStringInt, bytesToHex, hexToBytes, sha1BytesSync } from '../helpers'
...
@provide(TYPES.MtpRsaKeysManager)
export default class MtpRsaKeysManager implements ag.MtpRsaKeysManager {
  private publicKeysParsed: { [key: string]: any } = {}
  ...
  public prepareRsaKeys () {
    if (this.prepared) {
      return
    }
    for (const keyParsed of publisKeysHex) {
      const RSAPublicKey = this.createSerializer()
      RSAPublicKey.storeBytes(hexToBytes(keyParsed.modulus), 'n')
      RSAPublicKey.storeBytes(hexToBytes(keyParsed.exponent), 'e')
  ...
  public selectRsaKeyByFingerPrint (fingerprints): any {
    this.prepareRsaKeys()

    for (const fingerprint of fingerprints) {
      const fingerprintHex = bigStringInt(fingerprint).toString(16)
      const foundKey = this.publicKeysParsed[fingerprintHex]
      if (foundKey) {
        return { ...foundKey, fingerprint }
      }
    }
